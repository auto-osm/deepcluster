...
"""
def custom_greyscale_tensor(img, include_rgb=True):
  # Takes and returns a channel-first torch tensor

  # use channels last for cvtColor
  img_copy = np.array(img.numpy(), copy=True).transpose((1, 2, 0))
  h, w, c = img_copy.shape
  grey_img = cv2.cvtColor(img_copy, cv2.COLOR_RGB2GRAY)\
                .reshape(1, h, w).astype(np.float32)

  if include_rgb:
    img = np.concatenate([img, grey_img], axis=0)
  else:
    img = grey_img
...
def random_affine(img, min_rot=None, max_rot=None, min_shear=None,
  ...
  # https://github.com/pytorch/vision/blob/master/torchvision/transforms/functional.py#L623
  # RSS(a, scale, shear) = [cos(a) *scale   - sin(a + shear) * scale     0]
  #                        [ sin(a)*scale    cos(a + shear)*scale     0]
  #                        [     0                  0          1]
  # used by opencv functional _get_affine_matrix and skimage.transform.AffineTransform
...
"""
def random_translate_or_not(heatmaps, config):
  # before we realised that translation heatmaps takes up GPU memory
  # expects 4D tensor, we preserve gradients if there are any

  if (config.half_T_side_sparse_max != 0) or \
    (config.half_T_side_sparse_min != 0):
    ni, _, h, w = heatmaps.shape

    t = np.random.randint(config.half_T_side_sparse_min,
                           config.half_T_side_sparse_max + 1, size=(ni, 2))
    polarities = np.random.choice([-1, 1], size=(ni, 2), replace=True)
    t *= polarities

    # scale: in affine matrices scale, absolute value of 1 means half side
    t = t.astype(np.float32)
    t[:, 0] /= float(w / 2.)
    t[:, 1] /= float(h / 2.)

    tf_matrices = torch.zeros(ni, 2, 3).to(torch.float32).cuda()
    tf_matrices[:, 0, 0] = 1 # identity
    tf_matrices[:, 1, 1] = 1
    tf_matrices[:, :, 2] = torch.from_numpy(t).cuda()

    # actually does the inverse of t, but it's fine - opposite point across
    # the box/symmetric
    heatmaps = perform_affine_tf(heatmaps, tf_matrices)
...
def random_translation_multiple(data, half_side_min, half_side_max):
  ...
  data = F.pad(data, (half_side_max, half_side_max, half_side_max, half_side_max),
              "constant", 0)
